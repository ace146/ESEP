...
Serial::Serial(const char *dev) {	// /dev/ser4
	// Open File descriptor
	fdesc_ = open(dev, O_RDWR);
	if(fdesc_ < 0){
		throw std::logic_error("Failed to open "s + dev + " - " + std::strerror(errno));
	}
...
Serial::~Serial() {
	...
	if( close(fdesc_) < 0 ){
		throw std::logic_error("Failed to close serial "s + std::strerror(errno));
	}
}
...
int Serial::recv(uint32_t* p, size_t length){
	size_t received = 0;
	do {
		// return with up to sizeof(char) bytes when 10000 * 0.1 has expired or sizeof(char) bytes available
		int res = readcond(fdesc_, p, sizeof(char), sizeof(char),0,10000);
		if (res == 0) {
			break;
		}
		else if( res < 0){
